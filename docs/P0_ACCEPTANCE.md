...
## G2. 赛后生命周期 CTA
- [ ] 赛事 `finished` 后参赛者状态显示“赛事已结束”，不继续显示“已报名”。
- [ ] `finished` 底部主 CTA 为“查看赛果”，进入排名/赛果；“查看名单”只作为 Tab 历史信息，不再是赛后主任务。

## H. 移动端、语言与错误
- [ ] 登录前隐私政策无需登录即可访问；登录后通过“我的 → 设置与隐私 → 隐私政策”稳定进入同一份政策正文，不在“我的”重复第二入口；设置与隐私可稳定返回“我的”，二级页无底栏时有明确主页出口。
