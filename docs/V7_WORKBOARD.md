...
| V7-BRAND-01B | Raster social preview / OG / WeChat | P1 | NEEDS_VERIFY | build/dev 前以 Node built-ins 从批准 asset family 生成 `public/share-cover.png`（1200×1200），无需新增 npm dependency；OG/Twitter metadata 已改用 PNG 并声明 image/png/尺寸；`unit-brand-assets.mjs` 会真实生成并校验 PNG signature、dimensions、metadata，生成物已 gitignore。Candidate 真微信抓取仍不在 Builder 范围 | Verifier 跑 affected-scope Unit/H5 build；通过后转 BROWSER_PENDING，Candidate 再做真实微信抓取/缓存验证 |
| V7-VENUE-01A | Standard Event venue location foundation | P2 | NEEDS_VERIFY | provider-neutral schema 已增加 `venue_name/address/latitude/longitude/place_id/provider`；旧 `venue` 与 `venue_name` 双向兼容；`save_event` create/edit 持久化静态位置并校验坐标成对/范围；Standard form 只有主动“添加场地定位”后才展开位置能力，只有再点“使用当前位置”才调用 geolocation；Quick 未接入；`unit-venue-location.mjs` 已加入 `test:unit` | Verifier 跑 affected-scope Unit/H5 build + migration contract；实现者不得写 VERIFIED |
| V7-VENUE-01B | Standard Event detail external-map handoff | P2 | TODO | provider-neutral `MapProvider.externalMapUrl()` 基础已存在，但详情页尚未展示地址/坐标导航入口 | Builder after higher-priority actionable work |
