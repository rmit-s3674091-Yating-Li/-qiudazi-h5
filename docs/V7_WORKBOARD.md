...
| V7-REPO-01F | Candidate Browser workflow/scripts legacy audit | P2 | NEEDS_VERIFY | affected-scope reference audit confirms `.github/workflows/candidate-browser-blackbox.yml` remains an active `release-candidate` push + `repository_dispatch(vercel.deployment.success)` + `workflow_dispatch` release workflow and directly executes both `qa/candidate-browser-blackbox.mjs` and `qa/candidate-full-lifecycle.mjs`; current `docs/BROWSER_BLACKBOX_BASELINE.md` and `docs/RELEASE_GOVERNANCE.md` explicitly name the same workflow/scripts as the exact-head Candidate Browser evidence path. These assets therefore have current test/release/canonical dependencies and are not deletion-eligible; retain as `LEGACY_REVIEW`. No runtime code/migration/canonical AC changed, so the low-level regression for this reference-only cleanup slice is the workflow→scripts→canonical reference-chain audit rather than a synthetic product test | Verifier independently re-check current reference graph；实现者不得写 VERIFIED |
| V7-REPO-01G | Post-deploy backlog note legacy audit | P2 | NEEDS_VERIFY | `docs/AUDIT_BACKLOG_SNAPSHOT_POSTDEPLOY_NOTE.md` 明确自述为 post-deploy remediation 周期中的 repository sync note，并指向 `feature/20260831-postdeploy-ui-quickstart-filter-qa`；该历史 remediation branch 当前仍存在，最新提交为 `c309dcbcb6336a5162ac02882eb569476b78a3f3`。虽然该 note 不是 live AUD source of truth，但无法证明其 remediation/audit 追溯价值已为零，因此不满足安全删除条件；按清理安全线保留并标 `LEGACY_REVIEW`。本切片不改 runtime/build/test/migration/canonical AC，低层 regression 为 note→historical branch reference-chain 复核，不制造产品测试 | Verifier 独立复核该 note 的追溯价值与当前引用链；实现者不得写 VERIFIED |
