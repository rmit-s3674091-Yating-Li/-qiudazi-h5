...
| V7-BRAND-01A | Canonical C logo consistency | P1 | NEEDS_VERIFY | `public/share-cover.svg` 已移除旧三圆点 pseudo-logo，并直接嵌入与 `public/brand-icon.svg` 相同的 canonical C geometry/sage/off-white 识别；`unit-brand-assets.mjs` 已接入 `test:unit`。Builder self-check only | Verifier：跑 Domain Unit + H5 build；确认 H5/brand-icon/share-cover 无第二套 Logo |
| V7-BRAND-01B | Raster social preview / OG / WeChat | P1 | TODO | 当前 `index.html` 的 OG/Twitter 仍指向 `/share-cover.svg`；需输出兼容 social crawler 的 raster share asset 并收口 metadata，Candidate 再做真实微信抓取验证 | Builder |
