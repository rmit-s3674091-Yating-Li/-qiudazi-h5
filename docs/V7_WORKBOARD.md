...
| V7-ID-01B | Nickname identity exchange/account switch | P1 | NEEDS_VERIFY | test-only `test-identity-exchange` Edge Function 已通过 service-role-only normalized nickname lookup 定位唯一 completed Profile，使用 Supabase one-time magic-link hash + client `verifyOtp` 恢复 canonical auth session；不返回 password/access/refresh token。DB 以 `btrim + lower` 为权威规范化并增加唯一索引；Unicode 不做 compatibility folding。`/login` 已接入已有昵称登录；失败保持 login boundary；`unit-identity-exchange.mjs` 已接入 `test:unit`。Builder 仅 self-check，尚未独立验证/真实 Integration | Verifier：跑 Domain Unit；在 isolated Supabase 可用时验证 migration replay、重复规范化昵称拒绝、existing/missing nickname exchange、失败不产生半登录、刷新后 canonical Profile 绑定；Browser 后验证 G→老郑数据隔离 |
| V7-PHOTO-01 | Event photo → personal album / save to phone | P1 | NEEDS_VERIFY | source→personal independent copy/import/delete chain already exists；本切片在赛事相册与“参与赛事相册”补显式单张“保存到手机”，优先 Web Share files，普通浏览器降级 anchor download，受限 WebView 再降级打开短时 signed photo；`unit-photo-save.mjs` 已接入 `test:unit`。Builder self-check only；未写 VERIFIED | Verifier：跑 Unit + Build；Browser/真机验证 iOS Safari/微信 WebView 两处单张保存与取消/降级路径；既有 DB asset-decoupling Integration 保持 affected-scope 证据规则 |
