...
| V7-REPO-01D | AUD-015 specialist workflow/script legacy audit | P2 | NEEDS_VERIFY | affected-scope reference audit confirms `.github/workflows/aud015-doubles-partner-blackbox.yml` is still an active workflow (`release-candidate` push + `workflow_dispatch`) and directly executes `qa/aud015-doubles-partner-blackbox.mjs`; executed migration `20260829161024_audit_backlog_source_of_truth.sql` also preserves `AUD-20260829-015 / legacy-015-doubles-withdrawal` in the historical audit registry. Under the cleanup safety line this pair cannot be proven free of workflow/release/audit-trace dependencies, so both are retained as `LEGACY_REVIEW`; no runtime code/migration was changed and no synthetic product regression is warranted | Independent Verifier re-check trigger/reference/audit-trace graph; Builder must not mark VERIFIED |
| V7-REPO-01E | Exploratory Browser workflow/script legacy audit | P2 | NEEDS_VERIFY | affected-scope reference audit confirms `.github/workflows/exploratory-browser-blackbox.yml` is an active `release-candidate`/`workflow_dispatch` workflow and directly executes `qa/exploratory-browser-blackbox.mjs`; `docs/EXPLORATORY_BLACKBOX_BASELINE.md` is a current specialized canonical that explicitly requires this workflow/script pair and same-SHA exploratory evidence before Release Gate. Under the cleanup safety line these assets have current test/release/canonical dependencies and therefore must be retained as `LEGACY_REVIEW`; no runtime code or migration changed, so no synthetic product regression was added | Independent Verifier re-check workflow→script→canonical/Gate reference graph; Builder must not mark VERIFIED |
| V7-REPO-01F | Repo remaining dead/legacy artifact cleanup | P2 | TODO | restore bundle、layered CSS、`source.bundle.b64`、AUD-015 pair 与 Exploratory Browser pair 已分别审计；其他历史 QA/workflow/docs 等候选仍未逐项证明无运行/构建/测试/release/审计追溯依赖 | Builder 后续逐项审计；不能证明则继续标 `LEGACY_REVIEW`，不得盲删 |
