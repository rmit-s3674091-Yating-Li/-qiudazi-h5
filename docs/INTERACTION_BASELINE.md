...
- 名单锁定后仍可查看名单，但退出能力关闭；
- 开赛后用户可继续通过赛事 Tab 查看名单/对阵/结果，底部主任务必须随赛事生命周期演进。
- `finished` 状态不再把“查看名单”作为底部主 CTA。参赛者状态改为“赛事已结束 / 你参加了本场赛事”，底部主 CTA 为“查看赛果”，进入排名/赛果 Tab；名单只保留为历史 Tab 信息。
...
## 8. 设置与隐私
登录前必须可以直接访问《球搭子隐私政策（测试版）》，不得要求先登录。登录后的“设置与隐私”是个人偏好页，与登录前隐私政策分离。登录后通过“我的 → 设置与隐私”稳定进入“隐私政策”；不在“我的”主菜单重复增加第二个政策入口。登录前与登录后均指向同一份公开政策正文，不能只依赖登录页才能找到政策。已登录二级页在无底部主导航时应有稳定返回路径，并提供回到赛事大厅的主页入口；登录前隐私政策和首次建档不暴露已登录主页入口。
