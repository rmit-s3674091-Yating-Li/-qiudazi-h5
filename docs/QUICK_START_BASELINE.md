...
## 4. 正常流程
**选择单/双打 → 选择参赛者 →（双打）确认队友 → 设置赛制/计分；城市、场地均 optional → 一键开赛 → 创建赛事并锁定名单 → 自动生成首次对阵 → 进入 Draw/Match。**

Quick 的最低前置仅为满足比赛类型最低人数的参赛者 + 比比赛制/计分规则。城市不得带必填星号，不得自动写入“北京”或任何推断城市。首次 draw 失败时保留同一 Event，进入“开赛未完成 / 恢复开赛”；恢复只重试 draw，不重复创建 Event。创建并成功生成对阵后进入最贴近下一步操作的 Draw/Match 路径：唯一 Match 可突出直接开始/查看该 Match；多 Match 赛事先进入 Draw。
...
15. 创建成功后进入 Draw/Match，而不是无意义返回管理页；
