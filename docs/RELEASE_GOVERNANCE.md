...
> 本文是「球搭子」发布分支、Candidate Freeze、Vercel Preview、黑盒验证、Release Gate 与 main/CloudBase 发布顺序的 canonical source。它定义工程发布治理，不改变产品业务规则。
...
2. **release-candidate 候选层**
   ...
   - 该移动触发 Vercel Git Integration 自动生成唯一候选 Preview。
...
`feature/* → PR → exact-head H5 Build Check → Candidate Freeze → release-candidate → exact-head Vercel Preview → 黑盒/Visual/English → Release Gate → merge main 决策 → CloudBase/正式发布`
...
- 若未来改变该策略，必须同步更新本文、`docs/ENVIRONMENT_BASELINE.md`、README、P0、审计治理和 CHANGELOG，并重新验证实际 Vercel behavior。
...
Candidate Freeze 后，任何新的代码、migration、canonical 文档提交都会产生新的 PR head，并自动使旧 candidate 失去 exact-head 资格。此时必须：暂停黑盒/Gate → 完成新 head CI → 再移动 `release-candidate` → 只测试新的 Preview。
...
- 若新 head 只包含发布治理/文档变化，也必须重新跑 exact-head CI；不能继承旧 SHA 的 green 结论。
...
- 黑盒任务先执行 Candidate Preflight，再读取 backlog 和开始页面测试。
- 没有 exact-head READY candidate 时返回 `WAITING_FOR_CANDIDATE`，不测试旧 Preview、不创建产品 AUD。
- 一轮测试锁定一个 deployment id / URL / SHA；测试过程中不得切换候选。
- 黑盒只测试和取证，不修代码、不推动 candidate branch、不 merge main。
...
- 对该 deployment 的真实黑盒 / Visual / English 证据；
- live backlog 与发布相关 P0/P1 状态；
- repo/live Supabase 与安全一致性证据。

缺少 candidate 或对应黑盒时，Gate 只能 `WAITING_FOR_CANDIDATE_OR_BLACKBOX`，不能把“没有 Preview”登记成产品缺陷。
...
- 若 candidate freeze 后确有必要更新代码或 canonical 文档，应接受旧 candidate 作废，并在新 exact-head CI green 后重新生成 candidate；不得为了节省一次 Preview 而测试旧 SHA。
...
任何发布模型或 Vercel 分支策略变化，必须在同一轮同步：

1. `docs/RELEASE_GOVERNANCE.md`；
2. `docs/ENVIRONMENT_BASELINE.md`；
3. `docs/AUDIT_AUTOMATION_GOVERNANCE.md`；
4. `docs/P0_ACCEPTANCE.md`；
5. README；
6. `vercel.json` / CI 等实际配置；
7. `CHANGELOG.md`；
8. 相关自动化 prompt。

禁止只更新 `vercel.json` 或只在聊天中约定。