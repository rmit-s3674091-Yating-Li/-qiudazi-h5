...
- Vercel project id/name、deployment id、Preview URL 属于运行时平台事实，不凭历史值长期硬编码；使用前从 Vercel 当前项目/部署列表重新读取。
...
任何基础配置发生变化（例如 Supabase project ref、GitHub repo、主分支、部署环境、关键 env var 名称）时，必须在同一轮完成：
