...
P2：移动端日期控件溢出；赛事相册导入我的参赛相册及保存手机；Quick 城市/场地 optional；无意义重新生成对阵；Hall 城市筛选；单循环轮次与跨页面状态一致性；测试阶段账号切换/退出登录；标准赛事创建/编辑支持可选场地定位。
...
### 4.1 标准赛事场地定位（Standard Event only）

标准赛事创建/编辑增加可选“场地定位”。该能力只用于标准赛事，不进入 Quick Start；Quick 继续保持城市 optional、场地 optional 的最短创建链路，不增加地图或定位步骤。

场地定位不是赛事创建必填项。用户可以仅填写场地名称而不提供定位；只有用户主动点击“添加场地定位”后才进入地图选点、地点搜索或“使用当前位置”等能力，不得在进入创建页时自动请求浏览器/设备定位权限。

场地展示信息与地理位置事实分离。建议数据模型至少独立表达：
- `venue_name`：场地展示名称；
- `venue_address`：可选结构化/展示地址；
- `venue_latitude`：可选纬度；
- `venue_longitude`：可选经度。

不得把用户实时位置作为持续跟踪数据。产品只保存用户主动确认的赛事场地静态位置。赛事详情在存在合法场地坐标时可提供“查看地图 / 导航”入口；没有定位的赛事按现有场地文本正常展示，不出现空地图或创建阻塞。

实现时需保持 create/edit UI、schema/migration、save_event/RPC、snapshot/detail、权限与测试链一致；后续地图供应商（如高德/腾讯/Apple Maps）属于实现选择，不改变上述产品语义。
...
纯规则逻辑进入 Unit；RPC/RLS/Storage/事务/identity/幂等进入 Integration；真实移动端交互、弱网、快速连续记分、相册导入/保存、Quick 退出/取消、账号退出/切换、标准赛事场地定位与详情导航进入 User Story Browser。谁实现谁不独立 VERIFIED。
