...
const allowRepoOnly = process.argv.includes('--allow-repo-only');
...
const repoOnly = [...byVersion.keys()].filter((v) => v >= exactFrom && !expected.has(v) && !snapshots.has(v));
const extra = allowRepoOnly ? [] : repoOnly;
const drift = [];
...
if (allowRepoOnly && repoOnly.length) {
  console.log(`Migration feature preflight PASS: ${expected.size} live exact migrations preserved; ${repoOnly.length} repo-only migration(s) allowed for local replay: ${repoOnly.join(', ')}.`);
} else {
