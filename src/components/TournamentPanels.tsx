...
function DrawPlaceholder({ s }: { s: Snapshot }) {
  ...
  return <div className="tournament-placeholder">
    <div className="placeholder-heading row">
      ...
      <div><strong>{title}</strong><p className="muted small">{language==="en"?"Matches will appear here after the roster is locked and the draw is generated.":"名单锁定并生成对阵后，比赛会直接填入这里。"}</p></div>
    </div>
    <div className="draw-skeleton" aria-hidden="true">
      <section><small>{knockout ? (language==="en"?"First round":"首轮") : (language==="en"?"Round 1":"第 1 轮")}</small><div className="skeleton-match"><i/><i/></div><div className="skeleton-match"><i/><i/></div></section>
      <span className="draw-connector">›</span>
      <section><small>{knockout ? (language==="en"?"Next round":"下一轮") : (language==="en"?"Round 2":"第 2 轮")}</small><div className="skeleton-match compact"><i/><i/></div></section>
    </div>
    <p className="placeholder-foot">{language==="en"?"No real matchups yet, so no players or scores are fabricated.":"现在还没有真实对阵，不展示虚构选手或比分。"}</p>
  </div>;
}
...
function RankingPlaceholder() {
  ...
  return <div className="tournament-placeholder ranking-placeholder">
    <div className="placeholder-heading row"><span className="placeholder-icon"><ListOrdered size={19}/></span><div><strong>{language==="en"?"Event standings":"赛事排名"}</strong><p className="muted small">{language==="en"?"Standings update automatically from real results after matches begin.":"比赛开始后，排名会根据真实赛果自动更新。"}</p></div></div>
    <div className="ranking-table-skeleton" aria-hidden="true"><div className="ranking-table-head"><span>{language==="en"?"Rank":"名次"}</span><span>{language==="en"?"Player":"参赛者"}</span><span>{language==="en"?"Played":"已赛"}</span><span>{language==="en"?"W/L":"胜负"}</span><span>{language==="en"?"Diff":"局差"}</span></div>{[1,2,3].map(n=><div className="ranking-table-row" key={n}><b>0{n}</b><i/><span>—</span><span>—</span><span>—</span></div>)}</div>
    <p className="placeholder-foot">{language==="en"?"The standings area is ready; no fake ranks appear before real results exist.":"排名区域已经就位；没有真实赛果前不生成虚假名次。"}</p>
  </div>;
}
...
export function PhotoPanel({ s, owner, onDone }: { s: Snapshot; owner: boolean; onDone: () => void; }) {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      await rpc("save_event_photo", { p_event_id: s.event.id, p_original: paths[0], p_watermarked: paths[1], p_version: s.photo?.version || 0 });
      saved = true;
      if (s.photo) await supabase!.storage.from("event-photos").remove([s.photo.original_url, s.photo.watermarked_url]);
      onDone();
  ...
  return <>
    {s.photo ? <><img className="photo" src={assetUrl(s.photo.watermarked_url, "event-photos")} alt={language==="en"?`${s.event.name} post-match photo with final standings watermark`:s.event.name + "赛后合影，含最终名次水印"}/><p className="muted small">{language==="en"?"Main event photo":"赛事主合影"} · {new Date(s.photo.uploaded_at).toLocaleDateString(language==="en"?"en-US":"zh-CN")}</p></> : <div className="photo-upload-stage"><span className="photo-upload-icon"><Camera size={28}/></span><h3>{language==="en"?"Keep this match in a photo":"把这场球，留在照片里"}</h3><p className="muted">{s.event.status === "finished" ? owner ? (language==="en"?"Choose a photo from your phone. An event watermark will be generated automatically after upload.":"从手机照片中选择一张本场合影。上传后会自动生成赛事水印。") : (language==="en"?"Waiting for the organizer to upload the event photo.":"等待组织者上传本场合影。") : (language==="en"?"After the event finishes, this area will hold the event photo.":"比赛结束后，这里会成为本场赛事的合影位置。")}</p><div className="photo-frame-preview" aria-hidden="true"><ImagePlus size={24}/><span>{language==="en"?"Main event photo":"赛事主合影"}</span></div></div>}
    ...
    {pending && <Confirm title={language==="en"?"Replace the main event photo?":"替换本场主合影？"} description={language==="en"?"The new photo will replace the current one and regenerate the event name, final standings and date watermark.":"新照片将替换原主合影，并重新生成赛事名称、最终名次和日期水印。"} onCancel={() => setPending(null)} onConfirm={() => { const file = pending; setPending(null); void upload(file); }} />}
  </>;
}
