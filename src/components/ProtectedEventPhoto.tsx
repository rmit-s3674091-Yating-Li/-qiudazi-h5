...
export function ProtectedEventPhoto({previewPath,originalPath,alt,allowOriginal=true}:{previewPath:string;originalPath?:string|null;alt:string;allowOriginal?:boolean}){
  ...
  return <div className="protected-photo">
    {originalUrl ? <><img className="photo" src={originalUrl} alt={alt}/><div className="photo-actions single"><button className="secondary photo-action-button" onClick={()=>setOriginalUrl(undefined)}><X size={17}/>{language==="en"?"Close HD view":"关闭高清查看"}</button></div></> : <>
      ...
      {allowOriginal&&originalPath&&<div className="photo-actions"><button className="secondary photo-action-button" disabled={busy||!previewUrl} onClick={()=>void openOriginal()}><Eye size={17}/>{busy?(language==="en"?"Authorizing HD…":"正在授权高清…"):(language==="en"?"View HD photo":"查看高清")}</button><button className="secondary photo-action-button" disabled={saving||!previewUrl} onClick={()=>void saveOriginal()}><Download size={17}/>{saving?(language==="en"?"Preparing save…":"正在准备保存…"):(language==="en"?"Save to device":"保存到手机")}</button></div>}
    </>}
