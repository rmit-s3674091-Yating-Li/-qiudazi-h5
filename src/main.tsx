...
import { Hall } from "./pages/HallPage";
...
import { TemporaryPartnerDetailPage } from "./pages/TemporaryPartnerDetailPage";
...
function HomeRedirect(){const params=new URLSearchParams(window.location.search);const claimToken=params.get("claim");if(claimToken&&uuidPattern.test(claimToken))return <Navigate to={`/claim-player/${claimToken}`} replace/>;const inviterId=params.get("connect");if(inviterId&&uuidPattern.test(inviterId))return <Navigate to={`/connect/${inviterId}`} replace/>;return <Navigate to="/events" replace/>;}
function App(){
  const location=useLocation();
  if(!configured)return <main className="app-shell setup page"><Brand/><h1>球场已准备好，<br/>还差在线连接。</h1><p>当前没有配置在线数据库。</p></main>;
  ...
  return <div className={"app-shell "+(tab?"with-nav":"")}><Routes>
    ...
    <Route path="/temporary-partners/:playerId" element={<IdentityGate><TemporaryPartnerDetailPage/></IdentityGate>}/>
    ...
    <Route path="/events/new" element={<IdentityGate><EventFormPage/></IdentityGate>}/>
