...
import { EventForm, PrivacyPage } from "./pages/Basics";
...
function HomeRedirect() {
  ...
  if (claimToken && uuidPattern.test(claimToken)) return <Navigate to={`/claim-player/${claimToken}`} replace />;
  const inviterId = params.get("connect");
  if (inviterId && uuidPattern.test(inviterId)) return <Navigate to={`/connect/${inviterId}`} replace />;
...
function App() {
  ...
  if (!configured) {
    return <main className="app-shell setup page"><Brand /><h1>球场已准备好，<br />还差在线连接。</h1><p>当前没有配置 Supabase 项目。</p></main>;
  }
