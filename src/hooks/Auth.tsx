...
interface AuthState { session: Session | null; profile: Profile | null; ready: boolean; busy: boolean; error: string; start: () => Promise<Profile>; refresh: () => Promise<void>; }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  const [session, setSession] = useState<Session | null>(null), [profile, setProfile] = useState<Profile | null>(null), [ready, setReady] = useState(false), [busy, setBusy] = useState(false), [error, setError] = useState("");
  ...
  function start() {
    ...
    flight.current = (async () => {
      ...
      if (!s) {
        const storageKey = "qiudazi_guest_credentials_v3"; let credentials: { email: string; password: string } | null = null;
        try { const raw = localStorage.getItem(storageKey); if (raw) { const saved = JSON.parse(raw) as { email?: unknown; password?: unknown }; if (typeof saved.email === "string" && typeof saved.password === "string") credentials = { email: saved.email, password: saved.password }; } } catch {}
        if (credentials) { const signInResult = await supabase.auth.signInWithPassword(credentials); if (!signInResult.error) s = signInResult.data.session; else { localStorage.removeItem(storageKey); credentials = null; } }
        if (!s) { const provision = await supabase.functions.invoke("guest-session", { body: {} }); if (provision.error) throw provision.error; const data = provision.data as { email?: unknown; password?: unknown; error?: unknown } | null; if (!data || typeof data.email !== "string" || typeof data.password !== "string") throw new Error(typeof data?.error === "string" ? data.error : (isEnglish()?"Could not create a guest test identity.":"游客身份创建失败")); credentials = { email: data.email, password: data.password }; localStorage.setItem(storageKey, JSON.stringify(credentials)); const signInResult = await supabase.auth.signInWithPassword(credentials); if (signInResult.error) throw signInResult.error; s = signInResult.data.session; }
        if (!s) throw new Error(isEnglish()?"Could not create the guest test identity. Refresh the page and try again.":"游客身份创建失败，请刷新页面重试");
      }
      setSession(s); const p = await repository.profile(); setProfile(p); writeCachedProfile(s.user.id, p); return p;
    })().catch((e) => { setError(explainError(e)); throw e; }).finally(() => { setBusy(false); flight.current = null; });
    return flight.current;
  }
  return <AuthContext.Provider value={{ session, profile, ready, busy, error, start, refresh }}>{children}</AuthContext.Provider>;
}
...
export function IdentityGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (!auth.ready || auth.busy || auth.error) return; if (auth.profile?.profile_status === "completed") return; auth.start().then((p) => { if (p.profile_status !== "completed") navigate("/profile?next=" + encodeURIComponent(location.pathname + location.search), { replace: true }); }).catch(() => {}); }, [auth.ready, auth.profile?.id, location.pathname]);
