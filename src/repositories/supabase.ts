...
const messages:Record<string,string>={
  PROFILE_REQUIRED:"请先完成我的打球档案",DUPLICATE_PLAYER:"这位球搭子已经在本场名单中，不能重复报名",ENTRY_SIZE:"请选择正确的参赛人数",PLAYER_FORBIDDEN:"只能选择我的打球档案或已添加的临时球搭子",SELF_REQUIRED:"自主报名需要包含你自己的打球档案",SIGNUP_DISABLED:"这场赛事暂未开放自主报名",WAITLIST_FULL:"正式名额和候补名额都已满",TOO_FEW_ENTRIES:"至少需要2个正式参赛单元",GROUP_SIZE:"每组至少2个参赛单元，晋级人数须少于该组人数",AUTH_REQUIRED:"请先完成我的打球档案",FORBIDDEN:"你没有执行这个操作的权限",VERSION_CONFLICT:"这份数据刚刚被更新，请刷新后再试",NICKNAME_REQUIRED:"请填写1–40字昵称",NICKNAME_TAKEN:"这个昵称当前无法使用，请换一个昵称",TEST_IDENTITY_UNAVAILABLE:"这个测试昵称暂时无法进入，请换一个昵称或稍后重试",TEST_IDENTITY_RETRY:"测试身份刚刚发生变化，请重新提交一次",ROSTER_LOCKED:"参赛名单已经锁定，不能再修改这项内容",PLAYER_HAS_HISTORY:"已有比赛记录的临时球搭子不能删除",SELF_PLAYER_PROTECTED:"我的打球档案不能删除",EVENT_HAS_HISTORY:"已有参赛记录的赛事不能删除",EVENT_NOT_FOUND:"没有找到这场赛事",LIMIT_BELOW_ROSTER:"名额不能少于当前正式参赛人数",ENTRY_TYPE_LOCKED:"已经有人报名，不能再切换单打或双打",INVALID_AVATAR:"头像文件不可用，请重新上传",INVITE_NOT_FOUND:"这个邀请不存在或已经失效",INVITE_CLOSED:"这个邀请已经结束，请让对方重新发送",SELF_INVITE:"不能接受自己发出的邀请",PLAYER_ALREADY_CLAIMED:"这份打球记录已经关联过了",SELF_PLAYER_REQUIRED:"没有找到你的打球档案，请重新进入后再试",PLAYER_CLAIM_CONFLICT:"你和这份历史记录在同一赛事里都有参赛记录，暂时无法自动关联",CITY:"城市请控制在30字以内",VENUE:"比赛场地请控制在120字以内"
};
...
export async function command(eventId:string,action:Record<string,unknown>){const c=client();const{data:sessionData}=await c.auth.getSession();if(!sessionData.session)throw new Error("请先完成我的打球档案");const{data,error}=await c.functions.invoke("tournament-command",{body:{event_id:eventId,...action}});if(error){let message=error.message;try{const context=(error as{context?:Response}).context;if(context){const body=await context.clone().json();if(body?.error)message=body.error;}}catch{}throw new Error(explainError(message));}return data as Snapshot;}
