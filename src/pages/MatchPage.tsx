...
export function MatchPage({mode="detail"}:{mode?:"detail"|"direct"|"live"}){
  ...
  const[confirmText,setConfirmText]=useState(""),[error,setError]=useState(""),[busy,setBusy]=useState(false),[confirm,setConfirm]=useState<(()=>void)|null>(null),[inputs,setInputs]=useState<{a:string;b:string;ta:string;tb:string}[]>([]),[formVersion,setFormVersion]=useState<number>(),[editing,setEditing]=useState(false),[optimisticSides,setOptimisticSides]=useState<Side[]>([]);
  const loaded=useRef(""),pointQueue=useRef<Side[]>([]),pointFlush=useRef(false),latestSnapshot=useRef<Snapshot|null>(null);
  const s=q.data,m=s?.matches.find(x=>x.id===matchId);

  useEffect(()=>{if(s)latestSnapshot.current=s},[s]);
  useEffect(()=>{if(!s||!m||loaded.current===m.id+mode)return;loaded.current=m.id+mode;const sets=s.set_scores.filter(x=>x.match_id===m.id).sort((a,b)=>a.set_no-b.set_no);setInputs(Array.from({length:s.event.best_of},(_,i)=>({a:sets[i]?String(sets[i].a_games_or_points):"",b:sets[i]?String(sets[i].b_games_or_points):"",ta:sets[i]?.a_tiebreak_points!=null?String(sets[i].a_tiebreak_points):"",tb:sets[i]?.b_tiebreak_points!=null?String(sets[i].b_tiebreak_points):""})));setFormVersion(m.version);setEditing(false)},[s,m,mode]);
  ...
  async function flushPointQueue(){
    ...
    try{
      while(pointQueue.current.length){
        ...
        const side=pointQueue.current[0];
        const next=await command(current.event.id,{type:"point",event_version:current.event.version,match_id:currentMatch.id,match_version:currentMatch.version,side});
        latestSnapshot.current=next;
        pointQueue.current.shift();
        setOptimisticSides([...pointQueue.current]);
        q.replace(next);invalidateEventReads();
      }
    }catch(e){
      pointQueue.current=[];setOptimisticSides([]);setError(explainError(e));void q.refresh();
  ...
  function enqueuePoint(side:Side){
    ...
    try{for(const queued of pointQueue.current){if(predicted.winner)return;predicted=addPoint(current.event,predicted,queued)}if(predicted.winner)return;addPoint(current.event,predicted,side)}catch{return}
    pointQueue.current.push(side);setOptimisticSides([...pointQueue.current]);void flushPointQueue();
  }
  ...
  let live=serverLive;
  try{for(const side of optimisticSides){if(live.winner)break;live=addPoint(e,live,side)}}catch{live=serverLive}
