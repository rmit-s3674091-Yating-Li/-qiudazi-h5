...
export function Hall({mine=false}:{mine?:boolean}){
  ...
  return <><Header title={mine?"我的赛事":"球搭子"} back={false} action={!mine?<button className="icon-button" aria-label="筛选赛事" onClick={()=>setFilter(true)}><SlidersHorizontal size={20}/></button>:undefined}/><main className="page">
    ...
    {q.loading&&!q.data?<Loading/>:q.data?.length?q.data.map(e=><div className="event-list-item" key={e.id}><EventCard event={e} manage={mine&&scope==="created"}/>{mine&&scope==="joined"&&e.visibility==="public"&&e.status==="signup"&&<div className="event-list-actions"><Link className="text-button" to={`/events/${e.id}/invite-friends`}><UserPlus size={15}/>喊球搭子一起来</Link></div>}</div>):!q.error&&<Empty title={mine?(scope==="created"?(status?"当前筛选下没有赛事":"还没有创建赛事"):(status?"当前筛选下没有赛事":"还没有参与赛事")):"暂时没有可发现赛事"}><p>{mine?(scope==="created"?(status?"换个赛事状态看看，或创建一场新的比赛。":"召集搭子，开始一场自己的比赛。"):(status?"换个赛事状态看看。":"完成报名后，赛事会出现在这里。收到的邀请可以在上方查看。")):"现在还没有可以发现的比赛。可以稍后再来，或去“我的赛事”看看自己的比赛。"}</p>{mine&&scope==="created"&&!status&&<Link className="button" to="/events/new">创建赛事</Link>}{!mine&&<Link className="button secondary" to="/my-events">去我的赛事</Link>}</Empty>}
