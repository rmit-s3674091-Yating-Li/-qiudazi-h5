...
export function MyTennisProfilePage(){
 const [player,setPlayer]=useState<Player|null>(null),[loading,setLoading]=useState(true),[editing,setEditing]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState("");
 const [level,setLevel]=useState(""),[city,setCity]=useState(""),[times,setTimes]=useState<string[]>([]),[pref,setPref]=useState("");
 async function load(){try{const ps=await repository.players();const p=ps.find(x=>x.player_type==="self")||null;setPlayer(p);if(p){setLevel(p.level||"");setCity(p.city||"");setTimes(p.play_times||[]);setPref(p.play_preference||"");}}catch(e){setError(explainError(e));}finally{setLoading(false)}}
 useEffect(()=>{load()},[]);
 async function save(){setBusy(true);setError("");try{await rpc("save_my_tennis_profile",{p_level:level||null,p_city:city||null,p_play_times:times,p_play_preference:pref||null});await load();setEditing(false);}catch(e){setError(explainError(e));}finally{setBusy(false)}}
 if(loading)return <><Header title="我的打球档案"/><main className="page"><Loading/></main></>;
 return <><Header title="我的打球档案"/><main className="page">
  <ErrorNotice message={error}/>
  {player && <>
   <div className="tennis-profile-head"><Avatar path={player.avatar_url} name={player.name} size={58}/><div className="grow"><strong>{player.name}</strong><p className="muted small">用于赛事名单和赛果展示</p></div><a className="icon-button" href={`#/players/${player.id}/edit`} aria-label="编辑头像和昵称"><Pencil size={17}/></a></div>
   ...
   {!editing ? <div className="profile-fields">
    <div><small>水平级别</small><strong>{level||"待完善"}</strong></div><div><small>常打城市</small><strong>{city||"待完善"}</strong></div><div><small>单双打偏好</small><strong>{pref==="singles"?"单打":pref==="doubles"?"双打":pref==="both"?"单双打都可以":"待完善"}</strong></div><div><small>约球时间</small><strong>{times.length?times.join("、"):"待完善"}</strong></div>
