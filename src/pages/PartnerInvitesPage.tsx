...
export function PartnerInvitesPage(){
  ...
  const invalidCount=(ordinary.data?.filter(x=>x.status==="cancelled"||x.status==="expired").length||0)+(claims.data?.filter(x=>x.status==="cancelled"||x.status==="expired").length||0);
  async function clearInvalid(){if(!invalidCount)return;if(!window.confirm(en?`Clear ${invalidCount} inactive invitation record${invalidCount===1?"":"s"}? Cancelled and expired invitations will be removed. This cannot be undone.`:`清理这 ${invalidCount} 条失效邀请记录？已取消和已失效的邀请都会被移除，此操作不可撤销。`))return;setClearing(true);setFeedback("");try{await rpc("clear_invalid_partner_invites");await Promise.all([ordinary.refresh(),claims.refresh()]);}catch(e){setFeedback(explainError(e));}finally{setClearing(false);}}
  ...
  return <><Header title={t("partnerInviteHistory")}/><main className="page">
    ...
    {firstLoading?<Loading/>:!hasAny?<Empty title={en?"No partner invitation history yet":"还没有球搭子邀请记录"}><p>{en?"Invite a new partner, or invite a temporary partner to join, and progress will appear here.":"邀请新的球搭子，或邀请临时球搭子加入后，进度会出现在这里。"}</p></Empty>:<>
      <div className="invite-history-tools">{invalidCount>0&&<button className="text-button" disabled={clearing} onClick={clearInvalid}>{clearing?(en?"Clearing…":"正在清理…"):(en?"Clear inactive invitations":"清理失效邀请")}</button>}</div>
