...
type PlayerFormLocationState={returnTo?:string};

export function PlayerFormPage(){
  const{id}=useParams();const navigate=useNavigate();const location=useLocation();const auth=useAuth();const{language}=useLanguage();const en=language==="en";
  const returnTo=(location.state as PlayerFormLocationState|null)?.returnTo;
  ...
  async function save(e:React.FormEvent){e.preventDefault();setBusy(true);setError("");try{let avatar=player?.avatar_url||null;if(avatarFile){const blob=await imageBlob(avatarFile,512);const{data}=await supabase!.auth.getSession();avatar=await uploadAsset(blob,"avatars",data.session!.user.id+"/"+crypto.randomUUID()+".jpg");}await repository.savePlayer(id||null,name,avatar,player?.version);invalidateQuery("players");if(id)invalidateQuery(`temporary-partner-${id}`);invalidateQueryPrefix("partner-detail-");await auth.refresh();if(player?.player_type==="self"){if(returnTo==="/my-tennis-profile")navigate(-1);else navigate("/my-tennis-profile",{replace:true});}else navigate("/players",{replace:true});}catch(e){setError(explainError(e));}finally{setBusy(false);}}
  async function remove(){try{await repository.deletePlayer(id!);invalidateQuery("players");invalidateQuery(`temporary-partner-${id}`);navigate("/players",{replace:true});}catch(e){setError(explainError(e));setConfirm(false);}}
