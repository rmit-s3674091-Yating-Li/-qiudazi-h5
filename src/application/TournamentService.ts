...
export function applyCommand(original: Snapshot, actorId: string, command: TournamentCommand, c: DomainContext): Snapshot {
  ...
  if(command.type==="withdraw") {
    ensure(e.event_mode==="quick","WITHDRAW_MODE","此入口仅用于 Quick 比赛退出");
    ensure(e.status==="signup"||e.status==="locked","WITHDRAW_STATE","比赛开始后不能普通退出，请使用退赛/弃权结果流程");
    ensure(!s.matches.some(played),"WITHDRAW_STARTED","已有真实比赛开始，请使用退赛/弃权结果流程");
    const entry=s.entries.find(en=>en.status==="confirmed"&&en.signup_user_id===actorId);
    ensure(entry,"NOT_PARTICIPANT","仅当前赛事的实际参赛者可以退出");
    ensure(actorId!==e.owner_user_id,"OWNER_USE_CANCEL","创建人请使用取消比赛");
    ensure(command.confirmed,"CONFIRM_WITHDRAW","退出后参赛关系将终止，请确认");
    entry.status="withdrawn";entry.waitlist_order=null;
    const remaining=s.entries.filter(en=>en.status==="confirmed").length;
    s.matches=[];s.set_scores=[];s.point_logs=[];s.entries.forEach(en=>(en.group_no=null));e.draw_generated=false;
    if(remaining<2){e.status="cancelled";e.cancelled_at=c.now();}
  } else {
    ensure(e.owner_user_id===actorId,"FORBIDDEN","仅赛事创建者可以管理");
