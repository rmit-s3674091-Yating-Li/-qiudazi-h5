...
Deno.serve(async(req)=>{
  ...
  const auth=req.headers.get("authorization"); if(!auth) return json(req,{error:"AUTH_REQUIRED"},401);
  const url=Deno.env.get("SUPABASE_URL")!, anon=Deno.env.get("SUPABASE_ANON_KEY")!, service=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
  const userClient=createClient(url,anon,{global:{headers:{Authorization:auth}}});
  const admin=createClient(url,service);
  const {data:{user}}=await userClient.auth.getUser(); if(!user) return json(req,{error:"AUTH_REQUIRED"},401);
  const {data:profile,error:profileError}=await userClient.rpc("ensure_profile"); if(profileError||!profile?.id) return json(req,{error:"AUTH_REQUIRED"},401);
  ...
  if(body.action==="personal_preview"||body.action==="personal_original"||body.action==="partner_preview"){
    ...
    if(original&&!row.can_view_original) return json(req,{error:"FORBIDDEN"},403);
    const path=original?row.original_url:row.watermarked_url;
    if(!path) return json(req,{error:"PHOTO_NOT_FOUND"},404);
    ...
    if(signError||!signed?.signedUrl) return json(req,{error:"PHOTO_PREVIEW_FAILED"},500);
    return json(req,{url:signed.signedUrl,expires_in:ttl});
  }

  if(body.action==="import_personal"){
    if(!body.photo_id) return json(req,{error:"INVALID_REQUEST"},400);
    const {data:prepared,error:prepareError}=await userClient.rpc("prepare_personal_album_import",{p_photo_id:body.photo_id});
    if(prepareError||!prepared?.length){const m=prepareError?.message||"";if(m.includes("ALREADY_SAVED"))return json(req,{error:"ALREADY_SAVED"},409);if(m.includes("FORBIDDEN"))return json(req,{error:"FORBIDDEN"},403);return json(req,{error:"PHOTO_NOT_FOUND"},404);}
    ...
    try{
      ...
      return json(req,{ok:true,asset:saved});
    }catch(e){if(copied.length)await admin.storage.from("event-photos").remove(copied);const m=e instanceof Error?e.message:String(e);if(m.includes("PHOTO_NOT_FOUND"))return json(req,{error:"PHOTO_NOT_FOUND"},409);if(m.includes("ALREADY_SAVED"))return json(req,{error:"ALREADY_SAVED"},409);return json(req,{error:"PERSONAL_IMPORT_FAILED"},500);}
  }
  ...
  if(body.action==="delete_personal"){
    ...
    if(assetError||!assetRows?.length||!assetRows[0].can_view_original) return json(req,{error:"FORBIDDEN"},403);
    const {data:deleted,error:deleteError}=await userClient.rpc("delete_my_personal_album_metadata",{p_asset_id:body.asset_id});
    if(deleteError||!deleted) return json(req,{error:"PERSONAL_DELETE_FAILED"},500);
    ...
    if(removeError){await admin.from("participant_album_photos").insert(row);return json(req,{error:"PERSONAL_DELETE_FAILED"},500);}
    return json(req,{ok:true});
  }

  if(body.action==="finalize_upload"){
    if(!body.event_id||!body.original_path||!body.preview_path) return json(req,{error:"INVALID_REQUEST"},400);
    const {data:event}=await admin.from("events").select("id,owner_user_id,status").eq("id",body.event_id).maybeSingle();
    if(!event||event.owner_user_id!==profile.id) return json(req,{error:"FORBIDDEN"},403);
    if(event.status!=="finished") return json(req,{error:"EVENT_NOT_FINISHED"},409);
    const expectedPrefix=`${user.id}/${body.event_id}/`;
    if(!body.original_path.startsWith(expectedPrefix)||!body.preview_path.startsWith(expectedPrefix)||body.original_path===body.preview_path) return json(req,{error:"INVALID_PHOTO"},400);
    ...
    if(saveError){await admin.storage.from("event-photos").remove([body.original_path,body.preview_path]);return json(req,{error:saveError.message||"PHOTO_SAVE_FAILED"},400);}
    return json(req,{ok:true,photo:saved});
  }

  if(body.action==="delete"){
    if(!body.photo_id||typeof body.version!=="number") return json(req,{error:"INVALID_REQUEST"},400);
    ...
    if(readError) return json(req,{error:"PHOTO_NOT_FOUND"},404);
    if(!existing) return json(req,{error:"VERSION_CONFLICT"},409);
    ...
    if(deleteError){const m=deleteError.message||"";if(m.includes("VERSION_CONFLICT"))return json(req,{error:"VERSION_CONFLICT"},409);if(m.includes("FORBIDDEN"))return json(req,{error:"FORBIDDEN"},403);return json(req,{error:"PHOTO_METADATA_DELETE_FAILED"},500);}
    if(!deleted) return json(req,{error:"VERSION_CONFLICT"},409);
    const row=deleted as SourcePhoto;
    const {error:removeError}=await admin.storage.from("event-photos").remove([row.original_url,row.watermarked_url]);
    if(removeError){await admin.from("event_photos").insert(row);return json(req,{error:"PHOTO_DELETE_FAILED"},500);}
    return json(req,{ok:true});
  }
