...
const schema = z.object({
  event_id: z.uuid(),
  type: z.enum(["draw", "unlock", "start", "finish", "begin", "point", "undo", "score"]),
...
async function findPointOperation(client: ReturnType<typeof createClient>, cmd: Command) {
  ...
  const { data, error } = await client.from("point_logs")
    .select("id,match_id,winner_side")
    .eq("id", cmd.operation_id)
    .maybeSingle();
  if (error) throw error;
  if (!data) return null;
  if (data.match_id !== cmd.match_id || data.winner_side !== cmd.side) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // A retried point command must be safe even when the previous HTTP response
    // was lost after the database commit. The operation UUID is also the Point
    // Log UUID, so the authoritative log remains the only score truth.
    ...
    const next = applyCommand(snapshot as Snapshot, profile.id, cmd, {
      id: () => {
        if (cmd.type === "point" && cmd.operation_id && firstId) {
          firstId = false;
    ...
    const result = await client.rpc("commit_tournament", {
    ...
    if (result.error) {
      // Concurrent duplicate requests may both pass the pre-check; the loser can
      // observe VERSION_CONFLICT after the winner committed. Re-read by the same
      // operation id before deciding whether this was a real conflict.
