...
declare v_me uuid:=public.current_profile_id(); v_row public.connection_invites%rowtype;
begin
...
returns jsonb language plpgsql security definer set search_path='public' as $$
declare v_me uuid:=public.current_profile_id(); v_inv public.connection_invites%rowtype; v_profile public.profiles%rowtype; v_relationship text:='none';
begin
  if v_me is null then raise exception 'PROFILE_REQUIRED'; end if;
...
returns jsonb language plpgsql security definer set search_path='public' as $$
declare v_me uuid:=public.current_profile_id(); v_inv public.connection_invites%rowtype; v_connection jsonb;
begin
  if v_me is null then raise exception 'PROFILE_REQUIRED'; end if;
  ...
  if v_inv.status in ('cancelled','expired') then raise exception 'INVITE_UNAVAILABLE'; end if;
...
returns jsonb language sql security definer set search_path='public' as $$
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'created_at',ci.created_at,'responded_at',ci.responded_at,'accepted_by_nickname',p.nickname,'accepted_by_avatar_url',p.avatar_url
