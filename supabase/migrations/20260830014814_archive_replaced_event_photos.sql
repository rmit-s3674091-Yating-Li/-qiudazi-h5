...
declare e public.events; existing public.event_photos; result public.event_photos;begin
 ...
 if existing.id is not null then insert into public.event_photo_archive(event_id,original_url,watermarked_url,uploaded_at,source_version,archived_by_user_id) values(existing.event_id,existing.original_url,existing.watermarked_url,existing.uploaded_at,existing.version,public.current_profile_id()); end if;
