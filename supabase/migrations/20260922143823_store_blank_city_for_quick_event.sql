...
begin
  ...
  values(me,event_name,'public',false,match_type,fmt,1,scoring,null,case when scoring='games_4' then 4 when scoring='games_6' then 6 else null end,null,null,null,entry_count,event_day,event_clock,null,coalesce(city_text,''),venue_text,'free',null,null,null,null,null,null,'locked','quick') returning * into e;
  for raw_entry in select value from jsonb_array_elements(p_entries) loop
    select array_agg(value::uuid order by ordinality) into player_ids from jsonb_array_elements_text(raw_entry) with ordinality;
