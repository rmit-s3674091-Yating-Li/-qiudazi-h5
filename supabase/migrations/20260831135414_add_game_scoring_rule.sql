alter table public.events add column if not exists game_scoring text not null default 'advantage';
alter table public.events drop constraint if exists events_game_scoring_check;
alter table public.events add constraint events_game_scoring_check check (game_scoring in ('advantage','no_ad'));
...
begin
 ...
 if p_id is not null then
   ...
   if has_entries and (e.match_type!=old.match_type or e.format!=old.format or e.best_of!=old.best_of or e.scoring_type!=old.scoring_type or e.custom_games_target is distinct from old.custom_games_target or e.tiebreak_trigger is distinct from old.tiebreak_trigger or e.game_scoring is distinct from old.game_scoring or e.group_count is distinct from old.group_count or e.qualifiers_per_group is distinct from old.qualifiers_per_group) then raise exception 'EVENT_STRUCTURE_LOCKED'; end if;
   if old.status='locked' and (e.match_type!=old.match_type or e.format!=old.format or e.best_of!=old.best_of or e.scoring_type!=old.scoring_type or e.custom_games_target is distinct from old.custom_games_target or e.tiebreak_trigger is distinct from old.tiebreak_trigger or e.game_scoring is distinct from old.game_scoring or e.entry_limit is distinct from old.entry_limit or e.group_count is distinct from old.group_count or e.qualifiers_per_group is distinct from old.qualifiers_per_group) then raise exception 'EVENT_STRUCTURE_LOCKED'; end if;
   ...
   update public.events set name=e.name,visibility=e.visibility,link_signup_enabled=true,match_type=e.match_type,format=e.format,best_of=e.best_of,scoring_type=e.scoring_type,custom_games_target=e.custom_games_target,tiebreak_trigger=e.tiebreak_trigger,game_scoring=e.game_scoring,level=null,suggested_level_min=e.suggested_level_min,suggested_level_max=e.suggested_level_max,entry_limit=e.entry_limit,event_date=e.event_date,event_time=e.event_time,registration_deadline=e.registration_deadline,city=e.city,venue=e.venue,fee_type=e.fee_type,venue_fee_total=e.venue_fee_total,ball_fee_total=e.ball_fee_total,other_fee_total=e.other_fee_total,fixed_fee_per_entry=e.fixed_fee_per_entry,group_count=e.group_count,qualifiers_per_group=e.qualifiers_per_group,version=version+1 where id=p_id returning * into e;
 else
   insert into public.events(owner_user_id,name,visibility,link_signup_enabled,match_type,format,best_of,scoring_type,custom_games_target,tiebreak_trigger,game_scoring,level,suggested_level_min,suggested_level_max,entry_limit,event_date,event_time,registration_deadline,city,venue,fee_type,venue_fee_total,ball_fee_total,other_fee_total,fixed_fee_per_entry,group_count,qualifiers_per_group) values(me,e.name,e.visibility,true,e.match_type,e.format,e.best_of,e.scoring_type,e.custom_games_target,e.tiebreak_trigger,e.game_scoring,null,e.suggested_level_min,e.suggested_level_max,e.entry_limit,e.event_date,e.event_time,e.registration_deadline,e.city,e.venue,e.fee_type,e.venue_fee_total,e.ball_fee_total,e.other_fee_total,e.fixed_fee_per_entry,e.group_count,e.qualifiers_per_group) returning * into e;
 end if;
 return e;
end $function$;
