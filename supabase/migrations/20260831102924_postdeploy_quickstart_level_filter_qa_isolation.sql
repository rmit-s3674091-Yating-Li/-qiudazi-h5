create or replace function public.qiudazi_level_rank(p_level text)
returns integer
language sql
immutable
set search_path=''
as $$
  select case trim(p_level)
    when '≤2.0' then 20
    when '2.0' then 20
    when '2.5' then 25
    when '3.0' then 30
...
revoke all on function public.qiudazi_level_rank(text) from public, anon, authenticated;

create or replace function public.list_quick_start_players()
returns jsonb
language sql
stable
security definer
...
as $$
  with me as (
    ...
    select p.*, 'self'::text as quick_source, 0 as sort_order
    from public.players p
    cross join me
    where me.id is not null
      and p.owner_user_id=me.id
      and p.player_type='self'

    union all

    select p.*, 'connection'::text as quick_source, 1 as sort_order
    from public.players p
    cross join me
    where me.id is not null
      and p.owner_user_id<>me.id
      and p.player_type='self'
      and p.linked_user_id=p.owner_user_id
      and exists(
        select 1
        from public.connections c
        where c.status='accepted'
          and (
            (c.requester_user_id=me.id and c.addressee_user_id=p.owner_user_id)
            or (c.addressee_user_id=me.id and c.requester_user_id=p.owner_user_id)
          )
      )

    union all

    select p.*, 'temporary'::text as quick_source, 2 as sort_order
    from public.players p
    cross join me
    where me.id is not null
      and p.owner_user_id=me.id
  ...
  select coalesce(
    jsonb_agg(to_jsonb(a)-'sort_order' order by a.sort_order, lower(a.name), a.id),
    '[]'::jsonb
  )
  from allowed a;
$$;

revoke all on function public.list_quick_start_players() from public, anon;
grant execute on function public.list_quick_start_players() to authenticated, service_role;
          ...
          and (
            ...
            or (
              ...
              and exists(
                ...
                where c.status='accepted'
                  and (
                    (c.requester_user_id=me and c.addressee_user_id=p.owner_user_id)
  ...
  from(
    ...
    where(
      case when p_mine then
        case when p_filters->>'scope'='joined' then exists(
          select 1
          from public.entries own_entry
          join public.entry_players ep on ep.entry_id=own_entry.id and ep.active
      ...
      else e.status!='finished' and coalesce(e.event_mode,'standard')='standard' end
    )
    ...
    and(
      ...
      or (
        public.qiudazi_level_rank(p_filters->>'level') is not null
        and (e.suggested_level_min is null or public.qiudazi_level_rank(e.suggested_level_min)<=public.qiudazi_level_rank(p_filters->>'level'))
        and (e.suggested_level_max is null or public.qiudazi_level_rank(e.suggested_level_max)>=public.qiudazi_level_rank(p_filters->>'level'))
      )
    )
