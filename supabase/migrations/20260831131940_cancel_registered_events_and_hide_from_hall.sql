...
begin
 ...
 if exists(select 1 from public.entries where event_id=p_id) then
   update public.events set status='cancelled', cancelled_at=now(), version=version+1 where id=p_id;
   return;
 end if;
...
begin
  ...
  from(
    ...
    where(
      ...
      else e.status not in ('finished','cancelled') and (coalesce(e.event_mode,'standard')='standard' or (coalesce(e.event_mode,'standard')='quick' and e.visibility='public')) end
    )
