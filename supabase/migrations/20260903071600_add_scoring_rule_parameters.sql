alter table public.events add column if not exists tiebreak_target integer not null default 7;
alter table public.events add column if not exists tiebreak_win_by_two boolean not null default true;
alter table public.events add column if not exists games_win_by_two boolean not null default true;

alter table public.events drop constraint if exists events_tiebreak_target_check;
alter table public.events add constraint events_tiebreak_target_check check (tiebreak_target between 1 and 100);
...
begin
 ...
 if p_id is not null then
   ...
   update public.events set name=e.name,visibility=e.visibility,link_signup_enabled=true,match_type=e.match_type,format=e.format,best_of=e.best_of,scoring_type=e.scoring_type,custom_games_target=e.custom_games_target,tiebreak_trigger=e.tiebreak_trigger,tiebreak_target=e.tiebreak_target,tiebreak_win_by_two=e.tiebreak_win_by_two,games_win_by_two=e.games_win_by_two,game_scoring=e.game_scoring,level=null,suggested_level_min=e.suggested_level_min,suggested_level_max=e.suggested_level_max,entry_limit=e.entry_limit,event_date=e.event_date,event_time=e.event_time,registration_deadline=e.registration_deadline,city=e.city,venue=e.venue,fee_type=e.fee_type,venue_fee_total=e.venue_fee_total,ball_fee_total=e.ball_fee_total,other_fee_total=e.other_fee_total,fixed_fee_per_entry=e.fixed_fee_per_entry,group_count=e.group_count,qualifiers_per_group=e.qualifiers_per_group,version=version+1 where id=p_id returning * into e;
 else
   insert into public.events(owner_user_id,name,visibility,link_signup_enabled,match_type,format,best_of,scoring_type,custom_games_target,tiebreak_trigger,tiebreak_target,tiebreak_win_by_two,games_win_by_two,game_scoring,level,suggested_level_min,suggested_level_max,entry_limit,event_date,event_time,registration_deadline,city,venue,fee_type,venue_fee_total,ball_fee_total,other_fee_total,fixed_fee_per_entry,group_count,qualifiers_per_group) values(me,e.name,e.visibility,true,e.match_type,e.format,e.best_of,e.scoring_type,e.custom_games_target,e.tiebreak_trigger,e.tiebreak_target,e.tiebreak_win_by_two,e.games_win_by_two,e.game_scoring,null,e.suggested_level_min,e.suggested_level_max,e.entry_limit,e.event_date,e.event_time,e.registration_deadline,e.city,e.venue,e.fee_type,e.venue_fee_total,e.ball_fee_total,e.other_fee_total,e.fixed_fee_per_entry,e.group_count,e.qualifiers_per_group) returning * into e;
 end if;
 return e;
end $function$;
