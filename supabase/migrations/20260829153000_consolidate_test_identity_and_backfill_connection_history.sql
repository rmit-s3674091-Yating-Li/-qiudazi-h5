  ...
  for r in
    ...
    where (
      ...
      where p.profile_status = 'completed'
        and lower(trim(p.nickname)) = pn.normalized_nickname
    ) > 1
  loop
    ...
    where p.linked_user_id = r.canonical_id
...
-- Existing accepted Connections predate durable connection_invites. Backfill one
-- accepted history row so the user-facing invitation history remains complete.
