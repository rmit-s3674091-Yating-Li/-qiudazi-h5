...
as $$ declare e public.events; p public.event_photos; begin
...
as $$ declare me uuid:=public.current_profile_id(); begin
 if p_visibility not in ('private','partners') then raise exception 'INVALID_VISIBILITY'; end if;
 if me is null or not exists(select 1 from public.entries en join public.entry_players ep on ep.entry_id=en.id and ep.active join public.players p on p.id=ep.player_id where en.event_id=p_event_id and en.status<>'withdrawn' and p.linked_user_id=me) then raise exception 'FORBIDDEN'; end if;
 insert into public.event_album_preferences(profile_id,event_id,visibility,updated_at) values(me,p_event_id,p_visibility,now()) on conflict(profile_id,event_id) do update set visibility=excluded.visibility,updated_at=now();
 return p_visibility;
end $$;
...
as $$ with me as(select public.current_profile_id() id), participated as(select distinct en.event_id from me join public.players p on p.linked_user_id=me.id join public.entry_players ep on ep.player_id=p.id and ep.active join public.entries en on en.id=ep.entry_id where en.status<>'withdrawn') select e.id,e.name,e.event_date,ph.watermarked_url,ph.original_url,ph.uploaded_at,ph.event_visible,coalesce(pref.visibility,'private') from participated x join public.events e on e.id=x.event_id join public.event_photos ph on ph.event_id=e.id left join me on true left join public.event_album_preferences pref on pref.profile_id=me.id and pref.event_id=e.id order by e.event_date desc nulls last,ph.uploaded_at desc $$;
...
language sql stable security definer set search_path=''
as $$ with me as(select public.current_profile_id() id), allowed as(select me.id viewer_id from me where me.id is not null and exists(select 1 from public.connections c where c.status='accepted' and ((c.requester_user_id=me.id and c.addressee_user_id=p_profile_id) or (c.addressee_user_id=me.id and c.requester_user_id=p_profile_id)))), participated as(select distinct en.event_id from public.players p join public.entry_players ep on ep.player_id=p.id and ep.active join public.entries en on en.id=ep.entry_id where p.linked_user_id=p_profile_id and en.status<>'withdrawn') select e.id,e.name,e.event_date,ph.watermarked_url,ph.uploaded_at from allowed a cross join participated x join public.events e on e.id=x.event_id join public.event_photos ph on ph.event_id=e.id join public.event_album_preferences pref on pref.profile_id=p_profile_id and pref.event_id=e.id and pref.visibility='partners' order by e.event_date desc nulls last,ph.uploaded_at desc $$;
...
returns public.event_photos language plpgsql security definer set search_path=''
as $$ declare e public.events; result public.event_photos; begin
 select * into e from public.events where id=p_event_id for update;
 if e.id is null or e.owner_user_id is distinct from public.current_profile_id() then raise exception 'FORBIDDEN'; end if;
