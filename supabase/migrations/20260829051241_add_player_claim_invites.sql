create table if not exists public.player_claim_invites (
...
create index if not exists player_claim_invites_player_status_idx
  on public.player_claim_invites(player_id, status);
...
begin
  ...
  values (inv.inviter_user_id, me.id, 'accepted', now())
...
grant execute on function public.create_player_claim_invite(uuid) to authenticated;
grant execute on function public.get_player_claim_invite(uuid) to authenticated;
grant execute on function public.accept_player_claim_invite(uuid) to authenticated;
revoke all on function public.create_player_claim_invite(uuid) from anon;
revoke all on function public.get_player_claim_invite(uuid) from anon;
revoke all on function public.accept_player_claim_invite(uuid) from anon;
