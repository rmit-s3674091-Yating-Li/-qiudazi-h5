...
begin
  ...
  update public.events set status=next_state,draw_generated=(p_snapshot->'event'->>'draw_generated')::boolean,finished_at=(p_snapshot->'event'->>'finished_at')::timestamptz,version=version+1 where id=p_event_id returning version into v_next_version;
...
as $fn$
declare
  me uuid:=public.current_profile_id(); e public.events; raw_entry jsonb; player_ids uuid[]; player_id uuid; entry_row public.entries; expected integer; entry_count integer; seen uuid[]:=array[]::uuid[];
  fmt text:=coalesce(nullif(p_config->>'format',''),'round_robin'); scoring text:=coalesce(nullif(p_config->>'scoring_type',''),'games_6'); match_type text:=coalesce(nullif(p_config->>'match_type',''),'singles');
  city_text text:=nullif(trim(p_config->>'city'),''); venue_text text:=nullif(trim(p_config->>'venue'),''); event_name text:=nullif(trim(p_config->>'name'),'');
  event_day date:=coalesce(nullif(p_config->>'event_date','')::date,(now() at time zone 'Asia/Shanghai')::date); event_clock time:=coalesce(nullif(p_config->>'event_time','')::time,(now() at time zone 'Asia/Shanghai')::time(0));
begin
  ...
  entry_count:=jsonb_array_length(p_entries); if entry_count<2 then raise exception 'TOO_FEW_ENTRIES'; end if; expected:=case when match_type='doubles' then 2 else 1 end;
  for raw_entry in select value from jsonb_array_elements(p_entries) loop
    ...
    foreach player_id in array player_ids loop
      ...
      if not exists(select 1 from public.players p where p.id=player_id and ((p.owner_user_id=me and (p.player_type='self' or (p.player_type='manual' and p.linked_user_id is null))) or (p.owner_user_id<>me and p.player_type='self' and p.linked_user_id=p.owner_user_id and exists(select 1 from public.connections c where c.status='accepted' and ((c.requester_user_id=me and c.addressee_user_id=p.owner_user_id) or (c.addressee_user_id=me and c.requester_user_id=p.owner_user_id)))))) then raise exception 'PLAYER_FORBIDDEN'; end if;
  ...
  for raw_entry in select value from jsonb_array_elements(p_entries) loop
    select array_agg(value::uuid order by ordinality) into player_ids from jsonb_array_elements_text(raw_entry) with ordinality;
    insert into public.entries(event_id,entry_type,team_name,signup_user_id,status,waitlist_order) values(e.id,match_type,null,null,'confirmed',null) returning * into entry_row;
    insert into public.entry_players(entry_id,event_id,player_id,slot) select entry_row.id,e.id,x.id,x.slot::integer from unnest(player_ids) with ordinality as x(id,slot);
  end loop;
  return e;
end $fn$;
