...
5. `CHANGELOG.md` — 大版本、架构与部署节点历史
6. 当前已验证源码、共享组件与 `src/styles.css`
...
- 任何影响产品核心流程、信息架构、数据模型、权限、缓存/实时架构、身份、分享深链或部署策略的较大变更，都必须更新 `CHANGELOG.md`。
- 每个大版本记录至少包含：日期、分支/PR、主要变化、变化原因、兼容或数据影响、验证/部署状态、关键 commit 或最终部署 head。
- 小型文案、图标、间距和无行为影响的样式微调不要求逐条记录，避免日志噪声。
- 中国区手动部署前的综合审计应核对本轮是否已经形成 changelog；若没有，先补记录再进入部署测试。
- 部署验证完成后，应回填实际部署 head、测试结果和仍待解决问题；不要只记录“做过什么”，还要记录“哪一版真正被测试过”。
